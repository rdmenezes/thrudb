                              ...
                              BindResults * bind_results)
{
    ...
    T_DEBUG ("init: query=%s", this->query.c_str());
    ...
    if (this->stmt == NULL)
    {
        ...
        T_ERROR( "mysql_stmt_init failed: %p - %d - %s - %s", this->stmt,
                                   ...
                                   strlen (this->query))) != 0)
    {
        ...
        T_ERROR( "mysql_stmt_prepare failed: %d - %p - %d - %s - %s",
    ...
    if (this->bind_params != NULL)
    {
        ...
        {
            T_ERROR ("mysql_stmt_bind_param failed");
            ThrudocException e;
            e.what = "MySQLBackend error";
            throw e;
        }
    ...
    if (this->bind_results)
    {
                                    ...
                                    this->bind_results->get_results ()))
        {
            ...
            T_ERROR( "mysql_stmt_bind_result failed: %p - %d - %s",
...
void PreparedStatement::execute ()
{
    ...
    if (this->connection->get_read_only ())
    {
        if (this->writes)
        {
            ...
            T_DEBUG ("execute error: what=%s", e.what.c_str());
            throw e;
        }
    }
    ...
    if ((ret = mysql_stmt_execute (this->stmt)) != 0)
    {
        ...
        T_ERROR( "mysql_stmt_execute failed: %d - %p - %d - %s", ret,
    ...
    if ((ret = mysql_stmt_store_result (this->stmt)) != 0)
    {
        ...
        T_ERROR( "mysql_stmt_store_result failed: %d - %p - %d - %s", ret,
                 this->stmt, mysql_stmt_errno (this->stmt),
                 mysql_stmt_error (this->stmt));
...
void Connection::check_master ()
{
    ...
    if (this->read_only && this->read_only < time (0))
    {
        ...
        {
            T_DEBUG ( "check_master: failed, resetting read_only");
            this->read_only = time (NULL) + MYSQL_MASTER_RETRY_WAIT;
        }
    }
}
...
int PreparedStatement::fetch ()
{
    ...
    if (ret != 0 && ret != MYSQL_NO_DATA)
    {
        ...
        T_ERROR( "mysql_stmt_fetch failed: %d - %p - %d - %s",
...
void PreparedStatement::free_result ()
{
    ...
    if (ret != 0)
    {
        ...
        T_ERROR( "mysql_stmt_free_result failed: %d - %p - %d - %s",
                 ret, this->stmt, mysql_stmt_errno (this->stmt),
                 mysql_stmt_error (this->stmt));
                        ...
                        const char * password)
{
    ...
    T_DEBUG ("Connection: setting up master hostname=%s",hostname);

    if (!mysql_init (&this->mysql))
        T_ERROR ("mysql_init master failed");
    ...
    if (!mysql_real_connect (&this->mysql, hostname, username, password, db,
                             port, NULL, 0))
        T_ERROR ("mysql_real_connect master failed: host=%s, db=%s, user=%s",
                 hostname, db, username);
    ...
    if (slave_hostname && strlen (slave_hostname) > 0)
    {
        ...
        T_DEBUG("Connection: setting up slave hostname=%s",slave_hostname);

        if (!mysql_init (&this->slave_mysql))
            T_ERROR ( "mysql_init slave failed");
        ...
        if (!mysql_real_connect (&this->slave_mysql, slave_hostname,
            ...
            T_ERROR ("mysql_real_connect slave failed: host=%s, db=%s, user=%s",
                    hostname, db, username);
